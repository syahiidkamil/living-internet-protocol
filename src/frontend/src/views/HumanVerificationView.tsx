...
export const HumanVerificationView: React.FC<HumanVerificationViewProps> = ({
  ...
  const submitAnswer = async (answerIndex: number) => {
    ...
    try {
      const result = await backendService.verify_answer(
        currentChallenge.id,
        answerIndex,
      );
    ...
    <div className="mx-auto max-w-4xl p-8">
      ...
      {!currentChallenge && !showMintButton ? (
        <div className="text-center">
          <p className="mb-6">
            Complete {requiredChallenges} pattern challenge
            {requiredChallenges > 1 ? "s" : ""} to{" "}
            {mode === "refresh"
              ? "refresh your verification"
              : "prove you're human and mint your proof NFT"}
            .
          </p>
          <button
        ...
        <div className="text-center">
          ...
          <p className="mb-6">
            Score: {score}/{requiredChallenges}
          </p>
          <button
            onClick={mintProof}
            disabled={isLoading}
          ...
          <div className="space-y-6">
            <div className="text-center">
              <p className="mb-2 text-lg">
                Challenge {challengeNumber} of {requiredChallenges}
              </p>
            ...
            {showResult && (
              <div className="text-center">
                <p
                  className={`text-lg ${selectedAnswer === getCorrectAnswerIndex() ? "text-green-500" : "text-red-500"}`}
                >
                  {selectedAnswer === getCorrectAnswerIndex()
                    ? "✓ Correct!"
                    : "✗ Incorrect"}
                </p>
              </div>
